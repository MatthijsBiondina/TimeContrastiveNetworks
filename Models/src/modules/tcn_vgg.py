...
class GPU0(nn.Module):
    def __init__(self, state_dict_path=None):
        ...
        # Tools.pyout(models.vgg16().children())
        self.pretrained = nn.Sequential(
            *list(models.vgg16(pretrained=True).children())[0][:-8])
        #
        # self.pretrained = nn.Sequential(
        #     *list(models.vgg16(pretrained=True).children())[:-8])
        self.pretrained.eval()
        for ii, param in enumerate(self.pretrained.parameters()):
            # if ii < Config.TCN_INCEPTION_FIXED_LAYERS:
            param.requires_grad = False

        self.drop0 = nn.Dropout2d(p=0.2)

        self.conv1 = nn.Conv2d(512, 512, (3, 3), stride=1, padding=1)
        self.relu1 = nn.ReLU()
        self.bnrm1 = nn.BatchNorm2d(512)

        self.conv2 = nn.Conv2d(512, 512, (3, 3), stride=1, padding=1)
        self.relu2 = nn.ReLU()
...
class GPU1(nn.Module):
    def __init__(self, state_dict_path=None):
        ...
        self.relu2 = nn.ReLU()
        self.bnrm2 = nn.BatchNorm2d(512)
        self.spat2 = SpatialSoftmax(28, 28, 512, temperature=1.)
